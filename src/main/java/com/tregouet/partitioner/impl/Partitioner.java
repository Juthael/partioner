...
public class Partitioner<T> implements IPartitioner<T> {
	...
	/**
	 * https://en.wikipedia.org/wiki/Hierarchy_(mathematics) Spanning hierarchy :
	 * for n elements, contains (at least) the set of n elements and n sets of 1
	 * elements.
	 */
	@Override
	public List<List<List<T>>> getAllSpanningHierarchiesAsListsOfLists() {
		...
		if (order.size() == 1) {
			...
			for (List<List<T>> partition : getAllPartitions()) {
				if (partition.size() > 1) {
					/*
					 * LLL[L] - a sub-subset LL[L]L - a hierarchy of a subset L[L]LL - all
					 * hierarchies of a subset [L]LLL - for each subset of a partition, all
					 * hierarchies
					 */
					List<List<List<List<T>>>> hierarchiesOfEachSubset = new ArrayList<>();
					for (List<T> subset : partition) {
						List<List<List<T>>> hierarchiesOfCurrSubset = getNewPartitioner(subset)
								.getAllSpanningHierarchiesAsListsOfLists();
						hierarchiesOfEachSubset.add(hierarchiesOfCurrSubset);
					}
					/*
					 * For any partition P of a set S (with P ≠ S), a spanning hierarchy is S plus
					 * any element of the cartesian product of the lists of spanning hierarchies of
					 * P's subsets.
					 */
					for (List<List<List<T>>> forEachSubsetOneHierarchy : Lists
							.cartesianProduct(hierarchiesOfEachSubset)) {
	...
	public List<List<Set<T>>> getAllSpanningHierarchies() {
		...
		if (order.size() == 1) {
			...
			for (List<Set<T>> partition : getAllPartitionsAsListsOfSets()) {
				if (partition.size() > 1) {
					/*
					 * LLL[S] - a sub-subset LL[L]S - a hierarchy of a subset L[L]LS - all
					 * hierarchies of a subset [L]LLS - for each subset of a partition, all
					 * hierarchies
					 */
					List<List<List<Set<T>>>> hierarchiesOfEachSubset = new ArrayList<>();
					for (Set<T> subset : partition) {
						List<List<Set<T>>> hierarchiesOfCurrSubset = getNewPartitioner(subset)
								.getAllSpanningHierarchies();
						hierarchiesOfEachSubset.add(hierarchiesOfCurrSubset);
					}
					/*
					 * For any partition P of a set S (with P ≠ S), a spanning hierarchy is S plus
					 ...
					 * P's subsets.
					 */
					for (List<List<Set<T>>> forEachSubsetOneHierarchy : Lists
							.cartesianProduct(hierarchiesOfEachSubset)) {
